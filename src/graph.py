...
def comp_conf_interv(mcmc_array, quantile = [0.025,0.975], legend = ["PG", "IG", "EH"], color = ["red", "blue", "green"], nom_graph = 'Comparaison_individu_avec_intervalle_de_confiance', title = ''):
    ...
    ax.legend(loc='upper center', bbox_to_anchor=(0.5, -0.05),
                        fancybox=True, shadow=True, ncol=mcmc_array[0].shape[1])
    plt.savefig('../Images/'+nom_graph)
    return
    
def comp_scatter_plot(mcmc_array, legend = ["PG vs IG", "PG vs EH"], color = ["red", "blue"], combinaison =[[0,1],[0,2]], title = "Comparison between PG model and IG and EH model", nom_graph = "plot_meanvsmean"):
    fig, ax = plt.subplots()
    dic_mean = {i : [] for i in range(len(mcmc_array))}
    count_color = 0
    for method in range(len(mcmc_array)) :
